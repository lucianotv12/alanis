...
class Pedido {
    ...
    function nuevo_pedido($_PARAM){
        //idPedido, pedido, descripcion, activo, idEstado, idProveedor, fechaPedido, fechaRecepcion, idUsuario, contactoProveedor, metodoPedido
            ...
            $pedido->set_fechaPedido($_PARAM["fechaPedido"]);                
            $pedido->set_fechaRecepcion($_PARAM["fechaRecepcion"]);                
	...
	return($nrows);
        ...
        function actualiza_estado($idPedido,$idEstado)
        {
        ...
        function borrar_producto_pedido($idProducto_pedido)
        {

         // $query = "UPDATE pedidos set idEstado = $idEstado where idPedido = $idPedido";
          $query = " DELETE from pedidos_productos where idPedido_producto = $idProducto_pedido";
          mysql_query($query);


        }                

        function agregar_producto_pedido($_datos)
        {
            $datos = explode("_", $_datos);
            $idPedido = $datos[0];
            $idProducto = $datos[1];
            $detalleProducto = $datos[2];
            $cantidad = $datos[3];
         ...
         // $query = "UPDATE pedidos set idEstado = $idEstado where idPedido = $idPedido";
       //   $query = " DELETE from pedidos_productos where idPedido_producto = $idProducto_pedido";
            $query = "insert into pedidos_productos (idPedido_producto, idPedido, idProducto, producto, producto_relacion, cantidad, activo) values (0,'$idPedido','$idProducto','$detalleProducto','$relacion', '$cantidad','1')";
            mysql_query($query);
